        ...
        <div class="max-w-xs sm:max-w-sm md:max-w-xl lg:max-w-full lg:px-12 mx-auto">
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    ...
                    <h1 class="text-lg font-semibold leading-6 text-gray-700 dark:text-gray-100">{{ __('admin.partners_title') }}</h1>
                    <p class="mt-2 text-sm text-gray-700 dark:text-gray-100">
                        {{ __('admin.partners_description') }}
                    </p>
                    </div>
                    <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
                    <a type="button" class="primary-btn" href="{{ route('admin.partner.create') }}">
                        ...
                        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                            ...
                            @foreach($partners as $partner)
                                <tr>
                                    <td>
                                        @if($partner->logo) 
                                            <img src="{{ asset( 'pictures/'.$partner->logo ) }}" alt="" class="w-12 h-10 rounded-sm object-cover mr-2"/>
                                        @endif
                                    </td>
                                    <td class="table-text">
                                        {{ $partner->name }}
                                    ...
                                    <td class="relative py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                                        <a href="{{ route('admin.partner.edit', ['partner' => $partner->id]) }}" class="text-gray-400 hover:text-primary-500">
                                            <x-zondicon-edit-pencil class="w-4 h-4 text-primary-500 dark:text-white" />
                                        </a>
                                        <button onclick="openModal('{{ $partner->id }}')">
    ...
    <!-- <div id="myModal" class="hidden fixed z-10 inset-0 overflow-y-auto shadow-2xl"> -->
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="bg-white dark:bg-gray-400 rounded p-8 shadow-2xl">
                <!-- Modal content -->
                <h2 class="mb-2 text-base font-semibold text-gray-900 dark:text-gray-100">Are you sure you want to delete this partner?</h2>
                <form action="{{ route('admin.partner.delete', ['partner' => 'partnerId']) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<script>
    function openModal(partnerId) {
        ...
        form.action = form.action.replace('partnerId', partnerId);
