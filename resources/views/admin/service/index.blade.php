        ...
        <div class="max-w-xs sm:max-w-sm md:max-w-xl lg:max-w-full lg:px-12 mx-auto">
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    ...
                    <h1 class="text-lg font-semibold leading-6 text-gray-700">Services</h1>
                    <p class="mt-2 text-sm text-gray-700">
                        A list of all the services in your website.
                    </p>
                    </div>
                    <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
                    <a type="button" class="primary-btn" href="{{ route('admin.service.create') }}">
                        New Service
                    </a>
                    </div>
                </div>
                        ...
                        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                            ...
                            @foreach($services as $service)
                                <tr>
                                    <td>
                                        @if($service->image) 
                                            <img src="{{ asset( 'pictures/'.$service->image ) }}" alt="" class="w-12 h-10 rounded-sm object-cover mr-2"/>
                                        @endif
                                    </td>
                                    <td class="table-text">
                                        {{ $service->title_en }}
                                    ...
                                    <td class="relative py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                                        <a href="{{ route('admin.service.edit', ['service' => $service->id]) }}" class="text-indigo-600 hover:text-indigo-900">
    ...
    <!-- <div id="myModal" class="hidden fixed z-10 inset-0 overflow-y-auto shadow-2xl"> -->
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="bg-white rounded p-8 shadow-2xl">
                <!-- Modal content -->
                <h2 class="mb-2 text-base font-semibold text-gray-900">Are you sure you want to delete this service?</h2>
                <form action="{{ route('admin.service.delete', ['service' => 'serviceId']) }}" method="POST">
                    @csrf
                    @method('DELETE')
...
<script>
    function openModal(serviceId) {
        ...
        form.action = form.action.replace('serviceId', serviceId);
